...
class GCMC(nn.Module):
    ...
    def __init__(self, m_pi, n_dis, X_pi, X_dis):
        ...
        self.dense_x = nn.Linear(X_pi.shape[1], 1600)
        self.dense_y = nn.Linear(X_dis.shape[1], 1600)

        self.bm_x0 = norm.BatchNorm(1600)
        self.bm_y0 = norm.BatchNorm(1600)

        self.gcn_xy1 = dense.DenseGraphConv(1600, 1600)
        self.gcn_x1 = dense.DenseGCNConv(1600, 1600)
        self.gcn_y1 = dense.DenseGCNConv(1600, 1600)
        self.bm_xy1 = norm.BatchNorm(1600)
        self.bm_x1 = norm.BatchNorm(1600)
        self.bm_y1 = norm.BatchNorm(1600)
        ...
        self.gcn_xy2 = dense.DenseGraphConv(1600, 800)
    ...
    def forward(self, X_pi, X_dis, A_pi, A_dis, A_rel): #参数加了前两个
    # def forward(self, m_pi, n_dis, A_rel):
        """
        :param X_pi: m_pi x dim, feature matrix of piRNAs
        ...
        # Asso-GCN layer1 从gene和phenotype关联网络中提取特征
        XY = t.squeeze(F.leaky_relu(self.gcn_xy1(t.cat((X, Y)), A_rel)))
        XY = self.bm_xy1(XY)
        X, Y = t.split(XY, (m, n))
        # Sim-GCN layer1 从两个相似网络中提取特征
        ...
        # X, Y = t.split(XY, (m, n))
        # # Sim-GCN layer3
        # X = t.squeeze(F.leaky_relu(self.gcn_x3(X, A_pi)))
        # Y = t.squeeze(F.leaky_relu(self.gcn_y3(Y, A_dis)))
        # X = self.bm_x3(X)
        # Y = self.bm_y3(Y)
        #
        # # Asso-GCN layer4
        ...
        # X, Y = t.split(XY, (m, n))
        # # Sim-GCN layer4
        # X = t.squeeze(F.leaky_relu(self.gcn_x4(X, A_pi)))
        # Y = t.squeeze(F.leaky_relu(self.gcn_y4(Y, A_dis)))
        # X = self.bm_x4(X)
        # Y = self.bm_y4(Y)
