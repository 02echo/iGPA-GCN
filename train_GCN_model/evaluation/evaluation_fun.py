...
def plot_roc_curve(label, score):
    ...
    plt.plot([0, 1], [0, 1], color='gray', linestyle='--')  # 对角线（随机模型）
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
...
def plot_prc_curve(label, score):
...
def evaluation_all(score,label):
    ...
    # 使用你的 score 和 label 进行绘制
    plot_roc_curve(label, score)
    plot_prc_curve(label, score)
